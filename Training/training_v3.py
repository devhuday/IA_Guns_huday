...
#%%
ArchiveName = "GunshotIaModel_v3.h5"
...
def import_audios(audio_path, metadata, max_samples_per_class=3000, max_samples=3000 ):
    features, labels = [], []
    minForClass = 50
    ...
    for index, row in metadata.iterrows():
        ...
        if num_gun > max_samples and class_label == 1.0:
            continue
        if num_nogun > max_samples and class_label == 0.0:
            continue
        class_count[int(class_id)] += 1
        try:
    ...
    class_counts = {i: 0 for i in range(11)}  # De 0 a 13
    
    # Contadores para disparos y no disparos
    shots_count = 0
    non_shots_count = 0
    
    for class_id in range(14):
        class_data = metadata[metadata['classID'] == class_id]
        if class_id == 6:  # Disparos
            # Calculamos cuántas muestras base necesitamos considerando la aumentación
            # Como cada muestra se aumenta 3 veces, necesitamos max_samples_per_class/4 muestras base
            base_samples_needed = max_samples_per_class 
            samples = class_data.sample(min(base_samples_needed, len(class_data)), random_state=42)
        else:
            # Para las demás clases, tomamos muestras proporcionales
            available_slots = max_samples_per_class - non_shots_count
            if available_slots <= 0:
                continue
            samples_per_class = min(400, len(class_data), available_slots // (13))  # 13 clases no-disparo
            samples = class_data.sample(samples_per_class, random_state=42)
        ...
        for _, row in samples.iterrows():
            ...
            try:
                ...
                # Procesamiento para clase de disparos
                if class_id == 6:
                    if shots_count >= max_samples_per_class:
                        continue
                    ...
                    # Aumentación de datos solo si no excedemos el límite
                    for _ in range(3):
                        if shots_count >= max_samples_per_class:
                            break
                        augmented_audio = add_noise(librosa.load(file_name)[0])
                        augmented_spect = extract_spectrogram(file_name, duration=4)
                        features.append(augmented_spect)
                        labels.append(class_label)
                        shots_count += 1
                
                # Procesamiento para clases que no son disparos
                else:
                    if non_shots_count >= max_samples_per_class:
                        continue
                    features.append(data)
                    labels.append(class_label)
                    non_shots_count += 1
    ...
    print(f"Shots samples: {shots_count}")
    print(f"Non-shots samples: {non_shots_count}")
